...
std::array<std::uint64_t, 16> des_subkeys(std::span<const byte> key) {
  if (key.size() != 8) {
    fail("DES requires an 8-byte key");
  }
  ...
  for (std::size_t round = 0; round < subkeys.size(); ++round) {
    ...
    std::uint64_t joined = (static_cast<std::uint64_t>(c) << 28U) | d;
...
std::uint32_t des_feistel(std::uint32_t rhs, std::uint64_t subkey) {
  ...
  for (int box = 0; box < 8; ++box) {
    ...
    int row = ((chunk & 0x20U) >> 4U) | (chunk & 0x01U);
    int col = (chunk >> 1U) & 0x0FU;
...
std::uint64_t des_process_block(std::uint64_t block,
  ...
  for (int round = 0; round < 16; ++round) {
    std::uint32_t next_lhs = rhs;
    const auto &subkey = encrypt ? subkeys[round] : subkeys[15 - round];
    std::uint32_t next_rhs = lhs ^ des_feistel(rhs, subkey);
    lhs = next_lhs;
    rhs = next_rhs;
  }

  std::uint64_t preoutput = (static_cast<std::uint64_t>(rhs) << 32U) | lhs;
...
bytes des_block_encrypt(std::span<const byte> block,
  ...
  std::uint64_t value = des_process_block(load_be64(block), subkeys, true);
  bytes out(8);
...
bytes des_block_decrypt(std::span<const byte> block,
                        std::span<const byte> key) {
  const auto subkeys = des_subkeys(key);
  std::uint64_t value = des_process_block(load_be64(block), subkeys, false);
  bytes out(8);
  store_be64(value, out.data());
  return out;
}
...
bytes des_encrypt(std::span<const byte> input, std::span<const byte> key,
  ...
  return block_mode_apply(
      ...
      [&](std::span<const byte> block) {
        return des_block_encrypt(block, key);
      },
      [&](std::span<const byte> block) {
        return des_block_decrypt(block, key);
      },
      true);
}
